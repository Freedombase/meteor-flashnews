...
describe('freedombase:flashnews', function() {
  before('Prepare', function() {
    ...
    if (Meteor.isServer) {
      userId = Accounts.createUser({ username: 'test', email: 'test@meteor.io', password: 'test1234' })
    }
  })
  ...
  describe('adding', function() {
    it('fail on create', function() {
      ...
      if (Meteor.isServer) {
        ...
        const create1 = new Promise((resolve, reject) => Meteor.call('freedombase:flashnews-create', { en: 'fail' }, (error, response) => {
          resolve(error)
        }))
    ...
    it('create', function () {
      ...
      Meteor.call('freedombase:flashnews-create', { en: 'Hello universe!' }, 'en', function(error, result) {
        assert.isUndefined(error)
        assert.isDefined(result)
      })

      // Add with multiple languages
      Meteor.call('freedombase:flashnews-create', { en: 'Hello universe! 2', cs: 'Ahoj vesmire! 2' }, 'en', function(error, result) {
        assert.isUndefined(error)
        assert.isDefined(result)
      })
      ...
      Meteor.call('freedombase:flashnews-create', { en: 'Hello universe! 3', cs: 'Ahoj vesmire! 3' }, 'en', new Date(), undefined, APP_NEWS, undefined, undefined, undefined, ['cs'], function(error, result) {
        assert.isUndefined(error)
        assert.isDefined(result)
      })
      ...
      Meteor.call('freedombase:flashnews-create', { en: 'Hello from the future!', cs: 'Ahoj z budoucnosti!' }, 'en', future, function(error, result) {
        assert.isUndefined(error)
        assert.isDefined(result)
      })
